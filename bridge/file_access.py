...
def _snapshot_verified_handle(
    ...
    *,
    ...
    Verification and snapshot creation happen in one pass over the already
    descriptor-bound source.  Once the writer is closed, later writes through
    the registered pathname/inode cannot affect the returned stream.
    """
...
def open_verified_file(store: FileRecordStore, file_ref: str) -> VerifiedPrivateFile | None:
    ...
    identity on that exact descriptor. The bytes are copied while hashing into
    an unlinked private temporary file. Callers stream the snapshot, not the
    mutable registered inode, closing the post-verification mutation window.
    """
