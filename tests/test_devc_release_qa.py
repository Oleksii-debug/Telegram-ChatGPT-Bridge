...
class ReleasePackageIndependentTests(unittest.TestCase):
    def test_current_release_package_contract_ready_for_prepare(self):
        r=assess_release_root(ROOT); self.assertEqual("READY_FOR_PREPARE",r.status,r); self.assertEqual((),r.defect_codes); self.assertEqual((1,4),(r.direct_requirement_count,r.locked_requirement_count))

    def test_actual_wsgi_exact_and_negatives_fail_closed(self):
        source=(ROOT/"passenger_wsgi.py").read_text(encoding="utf-8"); self.assertEqual([],validate_passenger_wsgi_source(source))
    ...
    def test_dependency_actual_and_negative_matrix(self):
        req=(ROOT/"requirements.txt").read_text(encoding="utf-8"); lock=(ROOT/"requirements.lock").read_text(encoding="utf-8"); self.assertEqual(([],1,4),validate_dependency_envelope(req,lock)); h="c"*64
...
class PreparedAndCrossLaneTruthTests(unittest.TestCase):
    def test_prepared_metadata_positive_and_fail_closed(self):
        self.assertEqual([],validate_prepared_release_metadata(prepared_payload(),SHA)); p=prepared_payload(); p["sha"]="d"*40; p["requirements_lock_sha256"]="bad"; p["python_version"]="3.12.0"; p["immutable_permission_policy"]="mutable"; d=set(validate_prepared_release_metadata(p,SHA))
        self.assertTrue({"PREPARED_METADATA_STALE_SHA","PREPARED_METADATA_HASH_MISSING_OR_INVALID","PREPARED_METADATA_BUILT_PYTHON_INVALID","PREPARED_METADATA_IMMUTABILITY_POLICY_INVALID"}.issubset(d)); p=prepared_payload(); p["unexpected"]=True; self.assertEqual(["PREPARED_METADATA_SCHEMA_MISMATCH"],validate_prepared_release_metadata(p,SHA))

    def test_legacy_v1_remains_parseable_but_cannot_satisfy_strong_runtime_binding(self):
        payload=legacy_live_support_return(); self.assertEqual(1,validate_support_return(payload)["schema_version"]); readiness=build_deployment_readiness(payload)
        self.assertEqual("BLOCKED_EXTERNAL",readiness["checks"]["exact_candidate_runtime_binding"]["status"]); self.assertEqual("BLOCKED_EXTERNAL",readiness["checks"]["passenger_python_311"]["status"]); self.assertFalse(readiness["non_auditor_prerequisites_structurally_present"]); self.assertFalse(readiness["promotion_authorized"])

    def test_exact_v2_candidate_runtime_binding_is_structurally_accepted_but_never_authorizes(self):
        payload=bound_v2_support_return(); self.assertEqual(2,validate_support_return(payload)["schema_version"]); readiness=build_deployment_readiness(payload)
        self.assertEqual("PASS",readiness["checks"]["source_reconciliation"]["status"]); self.assertEqual("PASS",readiness["checks"]["exact_candidate_runtime_binding"]["status"]); self.assertEqual("PASS",readiness["checks"]["passenger_python_311"]["status"]); self.assertEqual("PASS",readiness["checks"]["backup_restart_identity_health_smoke_resume"]["status"]); self.assertEqual("PASS",readiness["checks"]["rollback"]["status"]); self.assertTrue(readiness["non_auditor_prerequisites_structurally_present"]); self.assertEqual("BLOCKED_EXTERNAL",readiness["checks"]["independent_auditor_gate"]["status"]); self.assertEqual("BLOCKED_EXTERNAL",readiness["checks"]["production_switch"]["status"]); self.assertFalse(readiness["promotion_authorized"])
...
class ProtocolTruthTests(unittest.TestCase):
    ...
    def test_nvda_is_human_live_only_and_gate_order_exact(self):
        steps=keyboard_nvda_protocol(); self.assertTrue(any("KEYBOARD_ONLY" in s for s in steps)); self.assertTrue(any("NVDA" in s for s in steps)); self.assertTrue(any("WITHOUT_MOUSE" in s for s in steps)); self.assertEqual(("INTERNAL_RELEASE_BLOCKER","HOSTIQ_LIVE","TELEGRAM_AUTH_E2E","DEPLOYED_ACTION","HUMAN_NVDA","K5"),remaining_gate_classes())
