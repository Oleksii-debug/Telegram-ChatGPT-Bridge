...
class Finalwave38SessionLockSecurityTests(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def test_symlink_parent_is_rejected_without_creating_target_lock(self) -> None:
        ...
        with self.assertRaisesRegex(SessionLockError, "parent_topology"):
            TelegramSessionLock(alias / "telegram-session.lock", timeout_seconds=0).acquire()
    ...
    def test_descriptor_binding_preserves_normal_mutual_exclusion(self) -> None:
