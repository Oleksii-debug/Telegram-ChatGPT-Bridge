...
def valid_live_binding(**overrides):
    data = {
        "deployed_sha": SHA_A,
        "release_gate_sha": SHA_A,
        "live_manifest_sha": SHA_A,
        "passenger_application_sha": SHA_A,
        "running_sha": SHA_A,
...
def ready_kwargs(**overrides):
    data = {
        ...
        "live_binding": valid_live_binding(),
...
class LivePrerequisiteBindingTests(unittest.TestCase):
    def test_valid_binding_keeps_every_live_stage_on_same_release(self):
        result = validate_live_prerequisite_binding(valid_live_binding())
        self.assertEqual(result.deployed_sha, SHA_A)
        self.assertTrue(result.release_identities_match())

    def test_binding_rejects_free_form_private_detail_fields(self):
        payload = valid_live_binding()
        payload["server_path"] = "/private/path"
        with self.assertRaises(HumanLiveGateError):
            validate_live_prerequisite_binding(payload)

    def test_binding_rejects_non_boolean_and_invalid_hash(self):
        with self.assertRaises(HumanLiveGateError):
            validate_live_prerequisite_binding(valid_live_binding(running_sha_verified=1))
        with self.assertRaises(HumanLiveGateError):
            validate_live_prerequisite_binding(valid_live_binding(setup_surface_sha256="short"))


class HumanLiveReadinessTests(unittest.TestCase):
    def test_source_green_alone_cannot_reach_human_gate(self):
        binding = valid_live_binding(independent_auditor_release_gate=False)
        result = evaluate_human_live_readiness(**ready_kwargs(live_binding=binding))
    ...
    def test_deployed_sha_must_equal_exact_source_release_sha(self):
        binding = valid_live_binding(
            deployed_sha=SHA_B,
            release_gate_sha=SHA_B,
            live_manifest_sha=SHA_B,
            passenger_application_sha=SHA_B,
            running_sha=SHA_B,
        )
        result = evaluate_human_live_readiness(**ready_kwargs(live_binding=binding))
    ...
    def test_stale_true_auditor_gate_cannot_be_reused_for_new_release(self):
        binding = valid_live_binding(
            deployed_sha=SHA_B,
            release_gate_sha=SHA_A,
            live_manifest_sha=SHA_B,
        ...
        result = evaluate_human_live_readiness(**ready_kwargs(source_sha=SHA_B, live_binding=binding))
        self.assertEqual(result.state, "BLOCKED_RELEASE_GATE_SHA_MISMATCH")
        self.assertFalse(result.ready_for_human_nvda)

    def test_stale_true_manifest_reconciliation_cannot_be_reused(self):
        binding = valid_live_binding(
            deployed_sha=SHA_B,
            release_gate_sha=SHA_B,
            live_manifest_sha=SHA_A,
        ...
        result = evaluate_human_live_readiness(**ready_kwargs(source_sha=SHA_B, live_binding=binding))
        self.assertEqual(result.state, "BLOCKED_LIVE_MANIFEST_SHA_MISMATCH")

    def test_stale_true_passenger_proof_cannot_be_reused(self):
        binding = valid_live_binding(
            deployed_sha=SHA_B,
            release_gate_sha=SHA_B,
            live_manifest_sha=SHA_B,
        ...
        result = evaluate_human_live_readiness(**ready_kwargs(source_sha=SHA_B, live_binding=binding))
        self.assertEqual(result.state, "BLOCKED_PASSENGER_SHA_MISMATCH")

    def test_stale_true_running_sha_proof_cannot_be_reused(self):
        binding = valid_live_binding(
            deployed_sha=SHA_B,
            release_gate_sha=SHA_B,
            live_manifest_sha=SHA_B,
        ...
        result = evaluate_human_live_readiness(**ready_kwargs(source_sha=SHA_B, live_binding=binding))
        self.assertEqual(result.state, "BLOCKED_RUNNING_SHA_MISMATCH")

    def test_cli_or_generic_runtime_is_not_passenger_application_process_proof(self):
        binding = valid_live_binding(passenger_application_process_verified=False)
        result = evaluate_human_live_readiness(**ready_kwargs(live_binding=binding))
    ...
    def test_running_sha_is_independent_required_gate(self):
        binding = valid_live_binding(running_sha_verified=False)
        result = evaluate_human_live_readiness(**ready_kwargs(live_binding=binding))
        self.assertEqual(result.state, "BLOCKED_RUNNING_SHA")

    def test_private_setup_surface_is_required(self):
        binding = valid_live_binding(private_setup_surface_ready=False)
        result = evaluate_human_live_readiness(**ready_kwargs(live_binding=binding))
    ...
    def test_human_nvda_can_be_ready_without_requesting_telegram_secrets(self):
        ...
        self.assertTrue(result.exact_deployment_bound)
    ...
    def test_invalid_source_boolean_fails_closed(self):
        with self.assertRaises(HumanLiveGateError):
