...
from bridge.final5_search_guard_r5 import GuardedTelethonReadBackend
...
class _RaisesInsideCall:
    ...
    def iter_messages(self, entity: object, *, limit: int, search: str = "", offset_id: int | None = None) -> list[object]:
...
class Final5Task3SearchGuardR5Tests(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def test_internal_typeerror_never_retries_without_constraints(self) -> None:
        ...
        self.assertEqual(client.calls, [{"entity": "peer", "limit": 25, "search": "needle", "offset_id": 77}])

    def test_supported_constraints_forward_exactly_once(self) -> None:
        client = _Supported()
        self.assertEqual(asyncio.run(self.backend._iter_messages(client, "peer", 25, search="needle", offset_id=77)), ["ok"])
        self.assertEqual(client.calls, [{"entity": "peer", "limit": 25, "search": "needle", "offset_id": 77}])
