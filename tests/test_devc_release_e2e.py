...
class ContentionTests(unittest.TestCase):
    def test_read_resume_duplicate_commit_exactly_one_effect(self):
        with tempfile.TemporaryDirectory() as t:
            ...
            flags=[]; busy_resumes=0; busy_writes=0
            for kind,r in results:
                ...
                if kind=="write" and r["status"].startswith("409"):
                    error=r.get("payload",{}).get("error",{})
                    self.assertEqual("write_in_progress",error.get("code"),(kind,r)); busy_writes+=1; continue
            ...
            self.assertGreaterEqual(busy_resumes,0); self.assertEqual(6,len(flags)+busy_writes)
            self.assertEqual(downloads,backend.download_count); self.assertEqual(1,len(fake.external_writes)); self.assertEqual(1,flags.count(False))
            final=request(app,"/api/v1/messages/send/commit",commit); self.assertTrue(final["status"].startswith("200"),final); self.assertTrue(final["payload"]["data"]["idempotent_replay"]); self.assertEqual(1,len(fake.external_writes))
