...
class ContentionTests(unittest.TestCase):
    def test_read_resume_duplicate_commit_exactly_one_effect(self):
        with tempfile.TemporaryDirectory() as t:
            ...
            flags=[]; busy_resumes=0; safe_write_conflicts=0
            allowed_write_conflicts={"write_in_progress","write_outcome_unknown_reconciliation_required"}
            for kind,r in results:
                ...
                if kind=="write" and r["status"].startswith("409"):
                    error=r.get("payload",{}).get("error",{})
                    self.assertIn(error.get("code"),allowed_write_conflicts,(kind,r)); safe_write_conflicts+=1; continue
            ...
            self.assertLessEqual(safe_write_conflicts,5)
            self.assertEqual(downloads,backend.download_count); self.assertEqual(1,len(fake.external_writes)); self.assertEqual(1,flags.count(False)); self.assertEqual(5-safe_write_conflicts,flags.count(True))
