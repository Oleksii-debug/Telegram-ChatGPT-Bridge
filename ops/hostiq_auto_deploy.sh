...
if [ "$ALLOW_PIP_INSTALL" = "1" ] && [ -f "$STAGE/requirements.txt" ]; then
  "$VENV_PYTHON" -m pip install -r "$STAGE/requirements.txt" >>"$LOG" 2>&1 || { say "pip failed for $SHA"; exit 0; }
fi
...
if ! curl -fsS --max-time 20 "$HEALTH_URL" >/dev/null; then
  ...
  mkdir -p "$APP/tmp"; touch "$APP/tmp/restart.txt"
  sleep 4
  curl -fsS --max-time 20 "$HEALTH_URL" >/dev/null && say "rollback health OK" || say "CRITICAL: rollback health also failed"
  exit 0
fi
