# -*- coding: utf-8 -*-
"""Fail-closed exact-blob adjudication for reviewed historical fixtures.

This module never changes current-tree scanning and never auto-approves a Git
history object.  A ledger entry can adjudicate only assignment-shaped findings
for one exact historical blob when path, full Git object id, SHA-256, and the
complete observed variable set all match.
"""
...
_ASSIGNMENT_FINDING_RE = re.compile(
    r"^history-blob:([0-9a-f]{40}): secret-like assignment "
    r"([A-Z][A-Z0-9_]{1,63}) in (.+)$"
)
...
def load_adjudications(repo: Path) -> dict[tuple[str, str, str], frozenset[str]]:
    """Load the strict history-only ledger, failing closed if it is malformed."""
...
def filter_exact_history_assignment_findings(
    *,
    ...
    """Return findings that remain after one exact history-only review.

    Private-key markers, setup routes, forbidden files, container failures,
    commit-message findings, current-tree findings, malformed ledgers, changed
    bytes, or any newly observed variable always remain failures.
    """
