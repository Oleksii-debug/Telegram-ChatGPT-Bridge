...
def _magic_archive_kind(data: bytes) -> str | None:
    if any(data.startswith(sig) for sig in ZIP_SIGNATURES):
        return "zip"
...
def _assignment_is_finding(name: str, value: str) -> bool:
    ...
    if stripped.casefold() in {"none", "null", "false", "true"}:
        return False
    if stripped in {"0", "1", "[]", "{}"}:
        return False
...
def scan_directory(root: Path, allowlist_repo: Path | None = None, scope: str = "directory") -> list[str]:
    ...
    for path in sorted(root.rglob("*")):
        if path.is_symlink():
            out.append(
                f"{scope}: symlink/uninspectable path: {path.relative_to(root).as_posix()}"
            )
            continue
        if not path.is_file():
            continue
